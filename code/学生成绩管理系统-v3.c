...
void modifyscore(){
    ...
    for(int i=0;i<student_count;i++){
        if(students[i].id==targetid){
            ...
            if(course_idx<1||course_idx>students[i].course_count){
                printf("无效序号！\n");
                break;
            }
            course_idx--;
            ...
            students[i].courses[course_idx].score=newscore;
            updateAverage(&students[i]);
            session_modify++;
            printf("成绩修改成功! 当前均分：%.2f\n", students[i].average);
            {char log_msg[60]; sprintf(log_msg, "修改了学生 %d %s 科成绩为 %.2f", targetid, students[i].courses[course_idx].name, newscore); writeLog(log_msg);}
            break;
        }
    }
    if(!found){
